...
public class AnimalController : MonoBehaviour
{
    ...
    private float mapRadius = 2000f;
    private float maxMapX = 2000f;
    private float maxmapZ = 2000f;
    private float minMapX = 0f;
    private float minMapZ = 0f;

    public float fleeingRange = 500;
...
//                        Vector3 spawnPosition = this.transform.position + new Vector3(0, 3, 0);
//                       GameObject baby = Instantiate(statsScript.child,spawnPosition,Quaternion.identity);
            GameObject baby = Instantiate(this.gameObject);
            birthing babyMaker = baby.GetComponent<birthing>();

            birthing thisMaker = this.GetComponent<birthing>();
            thisMaker.birth(father, statsScript);
...
//            Debug.Log( this.name + " carried to term");
 ...
 //       statsScript.isPregnant = false;
    ...
    private Vector3 wanderLocation()
    {
        ...
        {
            Vector3 randomDirection = new Vector3(Random.Range(minMapX, maxMapX), 0, Random.Range(minMapZ, maxmapZ));
            NavMeshHit hit;
...
//                            otherStatsScript.isPregnant = true;
    ...
    private void updateNaviation(animalStats.animalState currentState)
    {
        switch (currentState)
        {
            ...
            case animalStats.animalState.EATING:
                break;
            case animalStats.animalState.HAPPY:
    ...
    void Update()
    {
        ...
        {
            timeCounter = Time.time + refreshRate;
